        ...
        <div className='w-[25%] shadow-xl border-teal-600 rounded-r-md  md:w-[15%]  p-2   flex-col  border border-l-0  flex gap-2  mt-20 '>
          ...
          <div>
            <label  htmlFor='category' className='font-geist text-sm'>Category</label>
             ...
             <select   id='category' onChange={(e)=>router.push(`?category=${e.target.value}&sortBy=${sortBy?sortBy:"title"}&dis=${discount?discount:""}`)}  value={category?category:"all"} className=' text-xs   rounded-md w-full h-7   bg-slate-200 p-1 '>
          ...
          value={sortBy?sortBy:"title"}
          onChange={(e)=>router.push(`?category=${category?category:"all"}&sortBy=${e.target.value}&dis=${discount?discount:""}`)}
        >
                ...
                <form  className='ml-2'>
                   {
                    disvalues.map((item,index)=>(
                      ...
                      <input type='radio' name='discount' id={item.id} value={item.value}  checked={item.value==discount}
                      onChange={(e)=>router.push(`?category=${category?category:"all"}&sortBy=${sortBy?sortBy:"title"}&dis=${e.target.value}`)}
                      />
